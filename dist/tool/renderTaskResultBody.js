...
export function renderTaskResultBody(details, contentSummaryText, options, theme) {
    ...
    if (typeof details.tool_uses === "number" && details.tool_uses > 0) {
        const toolsLabel = details.tool_uses === 1
            ? "1 toolcall"
        ...
        container.addChild(new Text(" " + theme.fg("success", formatElapsed(durationMs)), 0, 0));
    }
    if (options.expanded) {
        if (structured) {
            ...
            if (tight) {
                ...
                if (line) {
                    const branchPreview = line.startsWith("⎿")
                        ? line
                        : `⎿ ${line}`;
                    container.addChild(new Text(theme.fg("dim", prefixResultLine(branchPreview, true)), 0, 0));
                }
            }
            else {
                for (const line of plainBody.split("\n")) {
                    container.addChild(new Text(theme.fg("dim", prefixResultLine(line, tight)), 0, 0));
                }
            }
        }
        else if (preview) {
...
function hasStructuredTaskDetails(details) {
    if (!details.structured_result)
        return false;
    return Boolean(details.findings ||
        details.evidence ||
        details.files ||
        details.caveats ||
        details.next_steps ||
...
function appendBlock(container, theme, label, body, tight) {
    const text = (body ?? "").trim();
    if (!text)
        return;
    container.addChild(new Text(theme.fg("toolTitle", ` ${label}`), 0, 0));
...
function parenthesizedExpandHint(action, theme) {
    const inner = expandCollapseHint(action);
    return theme.fg("dim", "(") + inner + theme.fg("dim", ")");
}
function pickLatestCollapsedPreviewLine(plainBody) {
...
function prefixResultLine(line, _tight) {

...
import { assertSdkToolCapability, buildAgentToolSelection, } from "./agent-tools.js";
import { BACKGROUND_CHECK_MS, COUNT_POLL_MS, MAX_POLL_ERRORS, TASK_TIMEOUT_MS, } from "./constants.js";
import { findJsonlSessionByName, normalizeConversationId, findTaskSessionHistory, mutateRegistry, readRegistry, readTaskSessionsRegistry, upsertTaskSessionHistory, writeTaskSessionsRegistry, } from "./conversation.js";
import { TASK_BACKGROUND_DEFAULT, buildPiArgs, buildTaskToolDescription, countToolUses, discoverAgents, subscribeToolEvents, resolveTaskAgentPreflight, assessTaskResult, buildTaskEnvelope, formatBackgroundReceipt, parseResultXml, shellQuote, } from "./helpers.js";
...
import { CHILD_CLAIM_GUARD_ENV, } from "./orchestration/runtime.js";
...
import { createDefaultHerdrTerminalBackend, createSyncHerdrControl, restoreHerdrWorkspaceGroups, } from "./subagent/herdr.js";
...
import { hasTmux, killAgentPane, paneExists, setPaneRemainOnExit, setPaneSelfDestruct, splitWindowPane, wrapWithPaneExitWatcher, } from "./subagent/tmux.js";
import { buildTaskPrompt, createTaskCompleteRenderer, listModelsParametersSchema, renderCall, renderResult, startForegroundProgressPolling, taskParametersSchema, } from "./tool/index.js";
import { ignoreStaleExtensionCtx } from "./stale-ctx.js";
import { serializeTaskAdmission } from "./task-admission.js";
import { resolveTaskCwd } from "./task-cwd.js";
import { createTaskWorktree, finalizeTaskWorktree, removeTaskWorktree, } from "./worktree.js";
// ─── Constants ───────────────────────────────────────────────────────────────
...
export default function (pi) {
    ...
    const unsubscribeTaskStopped = pi.events.on("pi-subagents:task-stopped", (payload) => {
    ...
    restoreHerdrWorkspaceGroups(readRegistry(piDir)
        .filter((entry) => entry.handle?.backend === "herdr" &&
        registryEntryStatus(entry) !== "missing")
        .map((entry) => entry.handle)
        .filter((handle) => handle?.backend === "herdr"));
    restoreActiveBackgroundTasks(piDir, backgroundTasks, registryEntryAlive, (entry) => {
        if (entry.handle?.backend === "herdr")
    ...
    // ── Widget / timer setup ───────────────────────────────────────────────
    const countInterval = startToolStatsPolling(foregroundTasks, backgroundTasks, COUNT_POLL_MS, taskWidget.requestRender);
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, params, signal, onUpdate, ctx) {
            ...
            const parentToolNames = pi
                ...
                .map((tool) => tool.name)
                .filter(Boolean);
            const preflight = resolveTaskAgentPreflight(agents, params.agent_type?.trim() ? params.agent_type : DEFAULT_TASK_AGENT);
            if (!preflight.ok) {
                return {
                    content: [
                        {
            ...
            if (params.cwd !== undefined) {
                ...
                if (requestedTaskCwd.kind === "invalid") {
                    return {
                        content: [{ type: "text", text: requestedTaskCwd.message }],
                        details: { phase: "failed", error: "invalid cwd" },
                        isError: true,
                    };
                }
            }
            let persistedTaskCwd;
            ...
            return serializeTaskAdmission(admissionKey, async () => {
                ...
                if (registeredTaskId) {
                    ...
                    if (activeTaskCwd.kind === "invalid") {
                        return {
                            content: [{ type: "text", text: activeTaskCwd.message }],
                            details: { phase: "failed", error: "invalid cwd", task_id: id },
                            isError: true,
                        };
                    }
                    ...
                    if (!entry) {
                        ...
                        if (entryStatus === "alive") {
                            if (params.background === false) {
                                return {
                                    content: [{ type: "text", text: `Task "${params.task_id}" is already running in the background and cannot be relaunched as foreground.` }],
                                    details: { phase: "failed", error: "active task cannot run foreground", task_id: id },
                                    isError: true,
                                };
                            }
                            ...
                            if (!steerResult.ok) {
                                return {
                                    content: [{ type: "text", text: `Task "${params.task_id}" was restored, but the follow-up prompt could not be delivered (${steerResult.reason}).` }],
                                    details: { phase: "failed", error: `resume steering failed: ${steerResult.reason}` },
                                    isError: true,
                                };
                            }
                ...
                if (worktree && !existsSync(worktree.path)) {
                    return {
                        content: [{ type: "text", text: `The isolated worktree for task ${id} no longer exists: ${worktree.path}` }],
                        details: { phase: "failed", error: "Task worktree missing", task_id: id },
                        isError: true,
                    };
                }
                if (!worktree && params.isolation === "worktree") {
                    try {
                        ...
                        return {
                            ...
                            details: { phase: "failed", error: message, task_id: id },
                            isError: true,
                        };
                    }
                }
                ...
                const claimGuardConfig = {
                    version: 2,
                    projectDirectory: taskCwd,
                    leaseStore: getOrchestrationPaths(ctx.cwd).leaseStore,
                    guardStatePath: join(getOrchestrationPaths(ctx.cwd).root, "claim-guards", `${params.__pi_subagents_invocation_id ?? id}.json`),
                ...
                try {
                    ...
                    claimGuardConfig.guardStateRequired = leases.some((lease) => claimOwnerIds.includes(lease.owner) &&
                        lease.claims.some((claim) => claim.kind === "write" || claim.kind === "test"));
                }
                catch {
                 ...
                 */
                const holdsSharedWriteClaims = async () => {
                    if (worktree)
                        return false;
                    try {
                        const leases = await listActiveResourceLeases({
                        ...
                        return leases.some((lease) => claimOwnerIds.includes(lease.owner) &&
                ...
                if (!["auto", "sdk", "tmux", "herdr"].includes(requestedBackend)) {
                    return {
                        ...
                        details: { phase: "failed", error: "invalid backend" },
                    };
                }
                ...
                if (!resume && typeof params.model === "string" && params.model.trim().length > 0) {
                    const knownModels = (subagentSettings.availableModels ?? []).map((m) => m.model);
                    if (knownModels.length > 0 &&
                        ...
                        return {
                            content: [
                                {
                                    type: "text",
                                    text: `Unknown launch model "${params.model.trim()}". Available models (subagents.availableModels):\n` +
                            ...
                            details: { phase: "failed", error: "unknown-model" },
                            isError: true,
                        };
                    }
                }
                const piArgs = buildPiArgs(agent, sessionName, sessionDir, promptContent, resume, parentToolNames, taskToolName, resumeSessionRef, promptLaunch, modelOverride);
                ...
                const runSdkFallback = async (foregroundTask, onSession) => {
                    ...
                    if (await holdsSharedWriteClaims()) {
                        throw new Error("This task holds write claims but the SDK backend cannot enforce them. " +
                ...
                if (useSdkBackend) {
                    if (isBackground) {
                        ...
                        ignoreStaleExtensionCtx(() => ensureTaskWidget(ctx));
                        const bgOnSession = (session) => subscribeToolEvents(session, backgroundTask, 10, taskWidget.requestRender);
                        ...
                        return {
                            content: [{ type: "text", text: formatSdkBackgroundReceipt(id) }],
                            details: {
                                phase: "running",
                                backend: "sdk",
                                background: true,
                ...
                try {
                    ...
                    return {
                        ...
                        details: { phase: "failed", error: `${selectedBackend} launch failed` },
                        isError: true,
                    };
                }
                    ...
                    try {
                        ...
                        try {
                            if (handle.backend === "herdr")
                                await herdrBackend.close(handle);
                            else
                                killAgentPane(paneId, originalPane);
                        }
                        catch {
                        ...
                        return {
                            content: [
                                { type: "text", text: "Foreground task failed before producing a result: " + message },
                            ],
                            details: {
                                phase: "failed",
                                execution_phase: "failed",
                ...
                if (handle.backend === "herdr" && herdrBackend.waitForAttention) {
                    void herdrBackend
                        ...
                        .catch((error) => {
                        if (herdrWaitController.signal.aborted)
                            return;
    ...
    pi.registerTool({
        name: "list_models",
        label: "list_models",
        description: "List the static set of models available for task launches (subagents.availableModels from pi settings) plus the configured default model. Use it to pick a valid value for the task tool's model argument.",
        promptSnippet: "List available launch models for the task tool (static list from settings)",
        ...
        async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
            ...
            if (settings.agentModels && Object.keys(settings.agentModels).length > 0) {
                lines.push("Per-agent-type defaults (subagents.agentModels):", ...Object.entries(settings.agentModels).map(([agentType, model]) => `- ${agentType}: ${model}`));
            }
            if (models.length === 0) {
                lines.push("(no models configured — add subagents.availableModels to .pi/settings.json or ~/.pi/agent/settings.json)");
            }
            else {
                lines.push("Available launch models (static list, subagents.availableModels):", ...models.map((m) => m.description ? `- ${m.model} — ${m.description}` : `- ${m.model}`));
            }
            if (settings.availableModelSkipped?.length) {
                lines.push("Skipped malformed settings entries:", ...settings.availableModelSkipped.map((s) => `! ${s}`));
            }
            ...
            lines.push(sourceLine, "", "Pass one of these (provider/model-id) as the model param when calling the task tool (optional — defaults apply when omitted).");
            return {
    ...
    pi.registerCommand("task-sessions", {
        ...
        handler: async (_args, ctx) => {
            ...
            ctx.ui.notify(rows.length > 0
                ? `Durable pi-task conversations:\n${rows.join("\n")}`
                : "No durable pi-task conversations found.", "info");
        },
    });
}

...
import { makeTaskSettledEvent, TASK_LIFECYCLE_EVENTS_V1, } from "@minhduydev/pi-core/task-lifecycle";
import { parseWorkflowCheckpoint, workflowCheckpointDigest, } from "@minhduydev/pi-core/workflow";
import { SUBAGENT_LEARNING_EVENTS_V1, makeProofVerifiedPayload, makeReviewCompletedPayload, } from "../events.js";
import { inspectTaskWorktree, mergeTaskWorktree, removeTaskWorktree, } from "../worktree.js";
import { releaseOrphanedLeases, releaseResourceLease } from "./claims.js";
import { buildContextPack, loadContextPack, saveContextPack, updateContextHandoff, } from "./context.js";
import { runOrchestrationDoctor } from "./doctor.js";
import { listEvidenceReceipts, recordEvidenceReceipt, verifyEvidenceReceipt, } from "./evidence.js";
...
import { isAcceptingReviewerVerdict, isReviewerEventBound, parseSemanticAttestations, parseReviewerOwnedVerdict, taskSubjectDigest, } from "./review.js";
import { getFinalTaskResult, getTaskSnapshot } from "./task-query.js";
import { getDurableRunByDecisionId, getDurableRunByTaskId, isTerminalExecutionPhase, listDurableRuns, patchDurableRun, } from "./run-store.js";
...
const TaskControlParameters = Type.Object({
    ...
    ceremony_steps: Type.Optional(Type.Array(Type.Object({
    ...
    evidence_kind: Type.Optional(Type.Union([
        Type.Literal("file"),
        Type.Literal("test"),
...
export function registerTaskControlTool(pi) {
    pi.registerTool({
        name: "task_control",
        label: "Task Control",
        description: "Query delegated task status/results, list recent tasks, append scoped handoffs or evidence, inspect local orchestration health, and perform explicit review or cleanup actions.",
...
async function executeHerdrAction(input, projectDirectory, pi) {
    ...
    switch (input.action) {
        ...
        case "handoff": {
            ...
            if (!checkpoint || checkpoint.kind !== "handoff") {
                throw new Error("handoff must be a canonical fourteen-section HandoffPackV1");
            }
            if (checkpoint.resumeKeys.taskId &&
                checkpoint.resumeKeys.taskId !== taskId) {
                throw new Error(`Handoff resume task ${checkpoint.resumeKeys.taskId} does not match ${taskId}`);
            }
        ...
        case "record_evidence": {
            ...
            const receipt = await recordEvidenceReceipt({
                ...
                description: requireValue(input.evidence_description, "evidence_description"),
        ...
        case "verify": {
            ...
            if (run.reportedOutcome !== "success") {
                throw new Error(`Task ${taskId} reported ${run.reportedOutcome}; only semantic success can be verified`);
            }
            ...
            const currentWorktree = run.worktree && run.worktreeDisposition === "retained"
            ...
            await patchDurableRun(paths.runStore, run.invocationId, {
                ...
                reviewPhase: (proof.valid || proof.receiptIntegrityValid) && run.verifier?.required
                    ? "awaiting"
            ...
            if (pi?.events) {
                const proofPayload = makeProofVerifiedPayload(taskId, proof.valid, proof.issues, (pack?.evidence ?? []).map((e) => e.sha256 ?? "").filter(Boolean), run.correlationId ?? run.invocationId, run.contextRequestDigest
                    ? {
        ...
        case "metrics": {
            ...
            return {
                content: [{ type: "text", text: renderMetrics(metrics) }],
                details: metrics,
            };
        }
        case "doctor": {
            ...
            return {
                content: [{ type: "text", text: renderDoctorResult(result) }],
                details: result,
            };
        }
        case "record_review": {
            ...
            const usageBindings = reviewedRun?.usageBindings && reviewedRun.usageBindings.length > 0
        ...
        case "review": {
            const taskId = requireValue(input.task_id, "task_id");
            const reviewerTaskId = requireValue(input.reviewer_task_id, "reviewer_task_id");
            ...
            if (!subject)
                throw new Error(`Subject task not found: ${taskId}`);
            if (!reviewer)
                throw new Error(`Reviewer task not found: ${reviewerTaskId}`);
            ...
            if (subject.reportedOutcome !== "success") {
                throw new Error(`Subject task ${taskId} reported ${subject.reportedOutcome}; non-success outcomes cannot be reviewed for shipment`);
            }
            if (subject.verificationPhase !== "passed" &&
            ...
            if (reviewer.reportedOutcome !== "success") {
                throw new Error(`Reviewer task ${reviewerTaskId} reported ${reviewer.reportedOutcome}`);
            }
            if (reviewer.verificationPhase !== "passed" &&
                reviewer.verificationPhase !== "not-required") {
                throw new Error(`Reviewer task ${reviewerTaskId} has not passed verification`);
            }
            if (subject.verifier?.reviewerAgent &&
                reviewer.agentType !== subject.verifier.reviewerAgent) {
                throw new Error(`Reviewer task must use agent ${subject.verifier.reviewerAgent}, got ${reviewer.agentType ?? "unknown"}`);
            }
            const subjectDigest = await taskSubjectDigest(projectDirectory, taskId);
            ...
            const reviewerVerdict = parseReviewerOwnedVerdict(reviewerOutput, subjectDigest);
            const verdict = reviewerVerdict.verdict;
            const subjectPack = await loadContextPack({
                storeDirectory: paths.contextStore,
                key: taskId,
            });
            const requiredSemanticClaims = verdict === "approved"
                ? [
                    ...(subjectPack?.claims ?? []),
                    ...(subjectPack?.learningClaims ?? []).map((claim) => claim.statement),
                ]
                : [];
            const semanticInputs = parseSemanticAttestations(reviewerOutput, requiredSemanticClaims, subjectDigest);
            ...
            for (const claim of subjectPack?.learningClaims ?? []) {
                learningClaimIds.set(claim.statement, [...(learningClaimIds.get(claim.statement) ?? []), claim.claimId]);
            }
            const semanticAttestations = semanticInputs.map((input) => {
                ...
                if (!receipt ||
                    ...
                    throw new Error(`Semantic attestation ${input.claim} does not reference a verified runtime receipt`);
                }
                return {
                    ...input,
            ...
            const existingReview = existingReviews.find((event) => event.type === "task_reviewed" &&
                event.taskId === taskId &&
                event.reviewerTaskId === reviewerTaskId &&
                event.subjectDigest === subjectDigest);
            if (existingReview?.verdict !== undefined &&
                (existingReview.verdict !== verdict ||
                    existingReview.reviewerOutputDigest !== reviewerVerdict.outputDigest)) {
                throw new Error(`Reviewer ${reviewerTaskId} already recorded an immutable verdict for this subject digest`);
            }
            ...
            if (!isAcceptingReviewerVerdict(verdict)) {
                ...
                await patchDurableRun(paths.runStore, subject.invocationId, (current) => {
                    ...
                    for (const attestation of semanticAttestations) {
                        const conflict = existing.find((candidate) => candidate.claim === attestation.claim &&
                        ...
                        if (conflict) {
                            throw new Error(`Semantic attestation for claim ${attestation.claim} is immutable`);
                        }
                    }
                    const merged = [
                        ...existing.filter((candidate) => !semanticAttestations.some((attestation) => attestation.claim === candidate.claim &&
            ...
            // Emit learning event after durable recording (fail-open)
            if (pi?.events) {
                const reviewPayload = makeReviewCompletedPayload(taskId, verdict, reviewerTaskId, reviewer.invocationId, subjectDigest, subject.correlationId ?? subject.invocationId);
                try {
        ...
        case "respond": {
            ...
            const run = await getDurableRunByDecisionId(paths.runStore, taskId, decisionId);
            if (!run)
                throw new Error(`Task not found: ${taskId}`);
            ...
            const resumeCorrelationId = decision.response?.resumeCorrelationId ??
            ...
            const transitioned = await patchDurableRun(paths.runStore, run.invocationId, (current) => {
                ...
                if (currentDecision.status === "resolved") {
                    if (currentDecision.response?.responseDigest !== responseDigest) {
                        throw new Error(`Decision ${decisionId} was already resolved with a different response`);
                    }
                    const recorded = currentDecision.response;
                    if (recorded?.resumedInvocationId ||
            ...
            if (!transitioned)
                throw new Error(`Task ${taskId} disappeared while recording decision ${decisionId}`);
            if (alreadyResolved) {
                return {
                    content: [
                        {
            ...
            try {
                ...
                const afterFailure = await patchDurableRun(paths.runStore, run.invocationId, (current) => {
                    const currentDecision = current.decisionRequest;
                ...
                if (durableResponse?.resumeState === "started") {
                    ...
                    throw new Error(`Decision ${decisionId} was recorded, but its task resume failed: ${resumeError}`);
                }
            }
            return {
                content: [
        ...
        case "ship": {
            ...
            if (subject.reportedOutcome !== "success") {
                throw new Error(`Task ${taskId} reported ${subject.reportedOutcome} and cannot ship`);
            }
            assertWorktreeMatchesVerifiedSnapshot(subject, taskId);
            ...
            if (subject.verificationPhase === "receipt-passed" &&
                (subject.verifier?.required || (subject.semanticAttestations?.length ?? 0) === 0)) {
                throw new Error(`Task ${taskId} has runtime receipt integrity but no reviewer-owned semantic proof`);
            }
            ...
            const boundReviews = await Promise.all(events.map(async (event) => isAcceptingReviewerVerdict(event.verdict) &&
                ...
                : undefined));
            const reviewers = new Set(boundReviews.filter((value) => value !== undefined));
            if (reviewers.size >= minReviews) {
                ...
                if (!events.some((event) => event.type === "task_completed" && event.taskId === taskId)) {
                    await appendOrchestrationEvent({
                        eventPath: paths.eventLog,
                        event: {
                            type: "task_completed",
                ...
                if (pi?.events) {
                    await pi.events.emit(TASK_LIFECYCLE_EVENTS_V1.SETTLED, makeTaskSettledEvent({
                        protocolVersion: 1,
                        taskId,
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `Shipped (${reviewers.size} independent review${reviewers.size === 1 ? "" : "s"}).`,
                        },
                    ],
        ...
        case "worktree_merge": {
            ...
            const events = await readOrchestrationEvents(paths.eventLog);
            if (!events.some((event) => event.type === "task_shipped" &&
                event.taskId === taskId &&
                event.subjectDigest === currentDigest)) {
                throw new Error(`Task ${taskId} must pass task_control ship for its current digest before merge`);
            }
            if (run.worktreeDisposition !== "retained") {
                throw new Error(`Task ${taskId} has no retained worktree to merge`);
            }
            const merged = mergeTaskWorktree(run.worktree, `Merge delegated task ${taskId}`, run.worktreeResult?.diffDigest);
            await patchDurableRun(paths.runStore, run.invocationId, {
        ...
        case "worktree_remove": {
            ...
            if (run.worktreeDisposition === "merged" || run.worktreeDisposition === "removed") {
                return {
                    content: [{ type: "text", text: `Task ${taskId} worktree is already ${run.worktreeDisposition}.` }],
            ...
            return {
                content: [{ type: "text", text: `Removed retained worktree for ${taskId}.` }],
        ...
        case "release": {
            ...
            const run = await getDurableRunByTaskId(paths.runStore, taskId);
            if (!run?.lease)
                throw new Error(`Task ${taskId} has no recorded lease`);
            ...
            if (!released) {
                return {
                    content: [
                        { type: "text", text: `Lease ${leaseId} was not active.` },
                    ],
            ...
            return {
                content: [
                    { type: "text", text: `Released lease ${leaseId}.` },
                ],
        ...
        case "reap": {
            ...
            for (const run of runs) {
                if (isTerminalExecutionPhase(run.executionPhase))
                    continue;
                if (now - Date.parse(run.heartbeatAt) >= staleAfterMs)
                    continue;
                aliveOwnerIds.add(run.invocationId);
                if (run.taskId)
                    aliveOwnerIds.add(run.taskId);
            }
...
async function taskListResult(paths) {
    const runs = (await listDurableRuns(paths.runStore))
        .slice()
    ...
    const lines = [
        `Durable task runs (${recent.length}${runs.length > recent.length ? ` of ${runs.length}` : ""}, most recently updated first):`,
        ...recent.map((run) => {
            const id = run.taskId ?? run.invocationId;
            const outcome = run.reportedOutcome && run.reportedOutcome !== "unknown"
...
function formatRunAge(isoTimestamp) {
    ...
    if (hours < 24)
        return `age ${hours}h ${minutes % 60}m`;
    const days = Math.floor(hours / 24);
    return `age ${days}d ${hours % 24}h`;
}
async function taskStatusResult(projectDirectory, taskId) {
    const snapshot = await getTaskSnapshot(projectDirectory, taskId);
    const paths = getOrchestrationPaths(projectDirectory);
    const run = await getDurableRunByTaskId(paths.runStore, taskId);
    const subjectDigest = run?.executionPhase === "completed"
...
async function taskFinalResult(projectDirectory, taskId) {
    const snapshot = await getTaskSnapshot(projectDirectory, taskId);
    if (!snapshot.sessionReference) {
        return {
            content: [
    ...
    const paths = getOrchestrationPaths(projectDirectory);
    const proofIssues = await detectWriteTaskProofFailure(projectDirectory, paths, taskId);
    if (proofIssues) {
        return {
            content: [
                {
...
async function detectWriteTaskProofFailure(projectDirectory, paths, taskId) {
    ...
    try {
        ...
        const isWriteAuthorized = pack?.authorization === "write-approved" ||
...
function findLastTaskEvent(events, taskId, types) {
...
function renderDoctorResult(result) {
    ...
    return [
        ...
        ...result.issues.map((issue) => `[${issue.severity}] ${issue.code}: ${issue.message} ${issue.remediation}`),
    ].join("\n");
}
async function requireRunWithOwnedWorktree(paths, taskId) {
    const run = await getDurableRunByTaskId(paths.runStore, taskId);
    if (!run)
        throw new Error(`Task not found: ${taskId}`);
    const handle = run.worktree;
    if (!handle)
        throw new Error(`Task ${taskId} has no recorded worktree`);
    const expectedRoot = resolve(dirname(handle.repositoryRoot), ".pi-subagents-worktrees", basename(handle.repositoryRoot));
    const relativePath = relative(expectedRoot, resolve(handle.path));
    if (!handle.branch.startsWith("pi-subagents/") ||
        relativePath === "" ||
...
function assertWorktreeMatchesVerifiedSnapshot(run, taskId) {
    if (!run.worktree || run.worktreeDisposition !== "retained")
        return;
    const current = inspectTaskWorktree(run.worktree);
    if (!run.worktreeResult || current.diffDigest !== run.worktreeResult.diffDigest) {
        throw new Error(`Task ${taskId} worktree changed after verification; run task_control verify again`);
    }
}
function assertRunReadyToMerge(run, taskId) {
    ...
    if (run.verificationPhase !== "passed" &&
...
async function ensureTaskContextPack(paths, projectDirectory, taskId, knownRun) {
